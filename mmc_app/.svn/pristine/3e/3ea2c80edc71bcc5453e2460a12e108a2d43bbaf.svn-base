...
void help_menu(){
	printf("Usage: mmcrandwrite [-secnduq] [arg]\n\n");
  printf("Perform random write, all the address is in sector mode.This function is used for random write performance test.\n\n");
  printf("Options:\n");
  ...
  printf("\t-n\tNUM\tspecify number of chunks to be random write\n");
  printf("\t-d\tNUM\tspecify const data\n");
  printf("\t-q\t   \tdon't print\n");
...
//  int time_flag=0;
  ...
  uint8_t cdata=0;
	...
	while(optind<argc){
    c = getopt(argc, argv,"s:e:c:n:d:qh");
   // printf("%c\n",c);
    switch(c){
    	...
    	case 'e':                               //end address
    	  ...
    	  break;
    	case 'n':                               //number of chunks to be write
  ...
  if(flashops->req(&req)<0){
  	printf("FLASHAPP ERR: mmc req failure.\n");
  	exit(1);
  }
  ...
  if(arg.mode == MODE_CONSTWT){
  	memset(data, cdata, sizeof(uint8_t)*chunk);
  }
  else{
  ...
  //gen a random addr array for random write access
  ...
  if(flashops->randwrite(data,addr,arg.special,chunk,req.data)<0){
  	printf("FLASHAPP ERR: mmcrandwrite failure.\n");
  	exit(1);
  }
